...
description: 'Configure Azure Active Directory SSO for Collate using SAML authentication'
---
...
<Accordion title="Key Notes on SAML Configuration">
...
- Edit `Basic SAML Configuration` and populate the values as shown below for `EntityId` and `Assertion Consumer Service Url`. These value should match the one configured with Collate Server side for `samlConfiguration.sp.entityId` and `samlConfiguration.sp.acs` respectively. After this click `Save`.
...
- To Confirm the claim value we can navigate to user page and check the value of the user. In my case as you can see User Principal Name is a my mail which i want to use for Collate, so for me `user.userprincipalname` would be correct claim.

<Accordion title="Certificate Configuration for Production">
Security requirements for your **production** environment:
...
### Step 2: Setup Collate Server

Open the downloaded metadata xml file, and populate the following properties in `collate.yml`:
...
<Accordion title="Security Parameters Configuration">
...
<Accordion title="JWT Security Requirements">
Security requirements for your **production** environment:
...
Start the Collate server. With `AUTHENTICATION_PROVIDER` set to saml, you should be routed to the IDP upon sign-in.